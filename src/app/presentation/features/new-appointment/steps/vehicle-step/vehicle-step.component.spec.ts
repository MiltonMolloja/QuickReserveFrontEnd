...
describe('VehicleStepComponent', () => {
  ...
  it('should set year to max when arrow key pressed on empty input', () => {
    const form = component.formGroup();
    expect(form.controls['year'].value).toBeNull();

    const input = fixture.nativeElement.querySelector('#vehicle-year') as HTMLInputElement;
    const event = new KeyboardEvent('keydown', { key: 'ArrowUp', cancelable: true });
    Object.defineProperty(event, 'target', { value: input });
    component['onYearSpin'](event);

    expect(form.controls['year'].value).toBe(VEHICLE_YEAR_MAX);
  });
